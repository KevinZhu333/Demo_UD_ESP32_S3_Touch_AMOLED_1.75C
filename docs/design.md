...
1. **主动先递，禁止「让用户问」**（RULE-1 / FR-A4 / FR-A7 / FR-B3）。系统灵魂是云端在 idle 命中那一刻把卡推下去，而非任何检索/问答入口。这条理念向下决定了：控制/推送面选 WSS 长连（云端可主动下发卡），Track B 证明音频按 ADR-0004 走本地 WAV 分段 + HTTP 完整段上传；MCP 续聊实现为「系统在 Yes 后递出的延续句柄」而非「用户枚举/检索记忆」。
...
│  职责: always-on 本地分段录音+后台上传(IN-2) / 1-bit 单卡+双击+超时+ABORT(IN-3,OUT-1) │
...
sequenceDiagram
  ...
  DEV->>GW: finalized WAV segment upload (IN-2, FR-B1)
  Note over DEV,GW: 网络中断→本地保留闭合分段, 恢复后重试到 checksum ACK (EDGE-6)
  GW->>PC: 完整音频分段
...
> 回答 spec §13 #2。设备是「哑终端 + 1-bit 阀门」：所有「该不该推、推什么、写哪里」都在云端；设备只负责①本地分段录音并后台上传完整音频分段、②在云端授权下亮一张卡、③把双击/超时/打断变成 1-bit 结果回云、④尽快熄屏。把智能放云端，是满足 RULE-2/RULE-8 的**结构性手段**——设备没有「对话/启动器」能力，就破不了这两条红线。
...
**固件模块（FreeRTOS 任务）**：M1 AudioCapture（I2S→本地 WAV 分段写入）、M2 SegmentUploader（闭合分段队列→HTTP POST→checksum/index ACK 后删除）、M3 LinkLayer（WSS 控制/状态/卡片推送，旧音频帧通道仅保留为非证明调试/未来可选模式）、M4 CardFSM（1-bit 卡状态机）、M5 InputSense（双击检测、VAD 开口、来电/来消息中断）、M6 Display（绘卡/熄屏/TTD 计时）、M7 AudioOut（A2DP 中继云端音频到现成耳机）、M8 Consent（物理采集开关 + 采集指示灯 + 云指令）、M9 SegmentQueue（SPIFFS/LittleFS 闭合 WAV 分段持久队列）。
...
- **传输**：控制面 + 推送面可走 **WSS** 长连（双向、低延迟，云端可主动下发卡）；Track B 音频证明路径按 ADR-0004 走「本地 WAV 分段录制 + HTTP 完整段上传」。实时 WSS 音频帧不是 Track B always-on recording 的证明要求。
...
{ "type":"HELLO", "device_id":"esp32-001", "device_class":"hw|sim",
  "fw_ver":"0.3.1", "epoch":42,
  "collection_token":"<signed-JWT>",   // ★ 无有效 ACTIVE token → Gateway 拒收音频
  "consent_telemetry":true,            // 仅遥测, 非授权依据 (P5)
  "caps":["audio.segmented_wav","card_control","a2dp","ble"] }
// Track B proof audio is uploaded out-of-band as complete WAV segments:
// POST /v1/device/audio-segments with run_id, segment_index, checksum, sample metadata.
...
{ "type":"AUDIO_ACK", "ack_seq":1390 }               // 仅旧实时帧/实验通道；Track B 分段上传以 HTTP 2xx + checksum ACK 为准
...
| 数据 | 持久化 | 丢失容忍 | 重连行为 |
|---|---|---|---|
| Track B WAV segment | SPIFFS/LittleFS durable segment queue | Proof runs zero normal loss: missing segment, storage overflow, checksum mismatch, or local gap fails proof | Retry upload until cloud ACKs `run_id + segment_index + checksum`, then delete local segment |
...
| 决策 | 选择 | 理由 | 被拒方案 |
|---|---|---|---|
| Track B 音频上行 | 本地 WAV 分段 + HTTP POST 完整段上传 | 证明目标是 always-on recording 质量与完整云端 artifact；上传 backpressure 不得竞争 I2S capture | 逐帧 WSS 音频帧（硬件测试已证明会损伤采集质量）、纯 MQTT（broker 运维/异栈） |
...
| IN-2 / FR-B1 | §2.3（HTTP audio segment upload）, §2.4, §4.1, ADR-0004 |
